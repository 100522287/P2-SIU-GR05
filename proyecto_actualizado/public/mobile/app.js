 ...
 * 
 * FIXES:
 * - Mejor integración de voz con feedback claro
 * - Gestos actualizados para reflejar nuevas interacciones de TV
 * - Comandos de voz emiten acciones directamente (no doble emisión)
 */
(function () {
  'use strict';
  ...
  function updateQueueUI(state) {
    ...
    mobileQueueList.querySelectorAll('.q-remove').forEach(btn => {
      btn.addEventListener('click', () => {
        socket.emit('remove-from-queue', parseInt(btn.dataset.index));
        if (navigator.vibrate) navigator.vibrate(30);
      });
    });
  }
  ...
  // =============================================
  // VOICE COMMANDS - FIX: emit action directly, not double
  // =============================================
  btnVoice.addEventListener('click', () => {
    ...
    if (voice.isListening) {
      ...
      setTimeout(() => btnVoice.classList.remove('listening'), 5000);
    }
  });
  ...
  voice.onCommand((cmd) => {
    ...
    // FIX: Emitir solo la acción directa, no gesture-detected + acción separada
  ...
  motion.onShake(() => {
    ...
    socket.emit('gesture-detected', {
      gesture: 'shake',
      action: 'none', // La acción ya se ejecutó con emit('skip')
  ...
  motion.onDrop(() => {
    ...
    socket.emit('gesture-detected', {
      gesture: 'drop',
      action: 'none',
      message: '⬇️ Descenso brusco → Silenciar/Activar sonido',
      icon: '⬇️',
      source: 'mobile-motion'
    });
  ...
  function showGestureFlash(icon, text) {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => gestureFlash.classList.add('hidden'), 300);
    }, 1500);
  }
