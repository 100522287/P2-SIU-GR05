...
let state = {
  ...
  isMuted: false,
  volume: 100,
  // Nuevo: estado de navegación en TV para selección por gestos
  tvBrowse: {
    active: false,
    selectedIndex: 0,
    category: 'all'   // 'all', 'pop', 'rock', 'fiesta', 'baladas'
  }
};
...
io.on('connection', (socket) => {
  ...
  socket.on('add-to-queue', (song) => {
    ...
    // Cerrar el menú de browse en la TV si estaba abierto
    state.tvBrowse.active = false;
  ...
  // --- Navegación por gestos en TV (browse catálogo) ---
  socket.on('tv-browse-toggle', () => {
    state.tvBrowse.active = !state.tvBrowse.active;
    state.tvBrowse.selectedIndex = 0;
    console.log(`[TV Browse] ${state.tvBrowse.active ? 'Activado' : 'Desactivado'}`);
    io.emit('sync-state', state);
  });

  socket.on('tv-browse-navigate', (direction) => {
    if (!state.tvBrowse.active) return;
    // direction: 'up', 'down', 'left', 'right'
    console.log(`[TV Browse] Navegar: ${direction}`);
    io.emit('tv-browse-nav', direction);
  });

  socket.on('tv-browse-select', () => {
    if (!state.tvBrowse.active) return;
    console.log('[TV Browse] Seleccionar canción');
    io.emit('tv-browse-confirm');
  });
  ...
  socket.on('gesture-detected', (data) => {
    ...
    switch (data.action) {
      ...
      case 'unmute':
        ...
        break;
      case 'confirm':
        // Confirmación - si browse está activo, seleccionar canción
        if (state.tvBrowse.active) {
          io.emit('tv-browse-confirm');
        }
        break;
      case 'restart':
      ...
      case 'browse-right':
        if (state.tvBrowse.active) {
          const dir = data.action.replace('browse-', '');
          io.emit('tv-browse-nav', dir);
        }
        break;
      case 'toggle-browse':
        state.tvBrowse.active = !state.tvBrowse.active;
